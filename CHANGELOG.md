...
- **GkCard docs playground:** GkCard docs now use reusable copyable example blocks (**`GkDocsExample`**, **`GkDocsCodeBlock`**) with syntax highlighting, long-code collapse/expand, raw sample-source imports (`?raw`), plus a lightweight editable playground that generates Vue code from selected options.
- **Containment docs examples:** Overlay, Dialog, BottomSheet, Menu, Tooltip, and NavigationDrawer docs now use focused raw-source live examples with copyable code. Dialog, BottomSheet, Menu, and Tooltip also include lightweight option playgrounds.
